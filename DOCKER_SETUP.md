# Docker Setup Guide for devOps-pj API

This guide explains how to run the devOps-pj API using Docker with different configurations for development and production environments.
...
docker exec devOps-pj-app-dev npm run db:migrate

# Open Drizzle Studio (inside running container)
docker exec devOps-pj-app-dev npm run db:studio
```
...
docker exec devOps-pj-neon-local psql -U neon -d neondb
```
...
docker exec devOps-pj-app-prod npm run db:migrate

# Or run one-time migration container
docker run --rm -it --env-file .env.production devOps-pj-app npm run db:migrate
```
...
docker exec devOps-pj-app-prod npm run db:studio
```
...
docker logs devOps-pj-neon-local
...
# Manual migration
docker exec -it devOps-pj-app-dev npm run db:generate
docker exec -it devOps-pj-app-dev npm run db:migrate
```
